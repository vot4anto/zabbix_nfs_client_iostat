...
# The number of failing connections (rpcinfo print those to STDERR, where others are reported to STDOUT), is the value the zabbix is monitoring on.
...
ERROR_COUNT=0
ERROR_DETAILS=""
for line in $(/usr/sbin/rpcinfo -p localhost | grep -v program | tr -s ' ' | cut -d' ' -f2- | sort -u | sed 's/ /,/g');
do
	program=`echo $line | cut -d',' -f1`
	version=`echo $line | cut -d',' -f2`
	port=`echo $line | cut -d',' -f4`
	service=`echo $line | cut -d',' -f5`
	rpcCmd=`/usr/sbin/rpcinfo -n $port -u localhost $program $version 2>&1 >/dev/null`
	if [ ! "$rpcCmd" = "" ]; then
